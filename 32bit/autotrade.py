...
def buy(past, cash, cash2, present_close, code, percent, segment_percent, trade_vol = -1) :
    ...
    if trade_vol != -1 :
        ...
        if volume == 0 :
            post_message("매수가능한 거래량이 0입니다.")
            return past, cash2
        if volume > cash/present_close*percent*segment_percent - past :
            volume = int(cash/present_close*percent*segment_percent - past)
            post_message("잔여예수금이 부족하여 해당 전략에 배정된 예수금을 모두 소진하였습니다.")
            if volume <= 0 :
                post_message("매수가능수량이 0보다 작아 해당 전략 하 매수를 종료합니다.")
                return past, cash2
        if(volume * present_close > cash2) :
            post_message("잔여예수금이 부족하여 해당 전략 하 매수를 종료합니다.")
            return past, cash2
...
def upper(past, cash, cash2, present_close, code, percent, segment_percent, trade_flag, max_price, final_flag, n=-1) :
    ...
    if (df.ma60.iloc[n] > df.ma120.iloc[n]) :
    ...
    if (max_price != 0) :
        max_price = max(max_price, df.close.iloc[n])

    if ((df.ma20.iloc[n] > df.ma120.iloc[n]*1.1) | (df.close.iloc[n] < max_price * 0.8)):
        final_flag = -1
        Neg_score = Neg_score + 0
            

    elif (df.ma20.iloc[n] <= df.ma120.iloc[n]) :
        final_flag = 1
        Pos_score = Pos_score + 4
        #print(df.ma20.iloc[i])
        #f2.write(str(df.ma120.iloc[i]) + "\n")

    if (df.close.iloc[n] < df.CLOSE_SIGNAL.iloc[n]) and (df.OBV.iloc[n] > df.OBV_SIGNAL.iloc[n]):
        Pos_score = Pos_score + 4
    ...
    if df.psychological_line.iloc[n] <= 0.3 :
        Pos_score = Pos_score + 0

    if df.psychological_line.iloc[n] >= 0.75 :
        Neg_score = Neg_score + 0
    ...
    if (df.MDI.iloc[n] >= df.PDI.iloc[n]) & (df.ADX.iloc[n] >= df.ADX.iloc[n-1]) & (df.ADX.iloc[n] >= df.PDI.iloc[n]) & (df.ADX.iloc[n] >= 30) :
        Pos_score = Pos_score + 0

    if (df.PDI.iloc[n] >= df.MDI.iloc[n]) & (df.ADX.iloc[n] >= df.ADX.iloc[n-1]) & (df.ADX.iloc[n] >= df.MDI.iloc[n]) & (df.ADX.iloc[n] >= 30):
         Neg_score = Neg_score + 4
                

    if (df.tenkan_sen.iloc[n] > df.kijun_sen.iloc[n]) & (df.low.iloc[n] > df.high.iloc[n-26]) & (df.close.iloc[n] > max(df.senkou_span_a.iloc[n], df.senkou_span_b.iloc[n])) :
        Pos_score = Pos_score + 6

    if (df.tenkan_sen.iloc[n] < df.kijun_sen.iloc[n]) & (df.high.iloc[n] < df.low.iloc[n-26]) & (df.close.iloc[n] < min(df.senkou_span_a.iloc[n], df.senkou_span_b.iloc[n])) :
        Neg_score = Neg_score + 4
        #f2.write(str(df.senkou_span_b.iloc[i]) + "\n")

    if (df.MACD.iloc[n] > df.SIGNAL.iloc[n]) and (df.MACD.iloc[n] < 0):
        Pos_score = Pos_score + 4

    if (df.MACD.iloc[n] < df.SIGNAL.iloc[n]) and (df.MACD.iloc[n] > 0):
        Neg_score = Neg_score + 4

    if (df.kdj_d.iloc[n] < 15) and (df.kdj_d.iloc[n] > df.kdj_j.iloc[n]):
        Pos_score = Pos_score + 2

    if (df.kdj_d.iloc[n] > 85) and (df.kdj_d.iloc[n] < df.kdj_j.iloc[n]):
        Neg_score = Neg_score + 2
        
    if n == -1 :
        post_message("upper전략을 실행합니다.")

    if (past == 0) and (Pos_score >= 4)  and (Neg_score < 6): 
        trade_flag = trade_flag + 1
        if trade_flag > 10 :
            trade_flag = 10
        if trade_flag == 10 :
            final_flag = -1
            max_price = df.close.iloc[n]
            if n == -1 :
                past, cash2 = buy(past, cash, cash2, present_close, code, segment_percent, 1)
            else :
                past = 1
        elif n == -1 :
            post_message("upper전략에서는 trade_flag를 증가시켰지만 금일 매수,매도가 이뤄지지 않았습니다")

    elif (final_flag == 1) and (Pos_score >= 4) and (Neg_score < 6):
        trade_flag = trade_flag + 1
        if trade_flag > 10 :
            trade_flag = 10
        if trade_flag == 10 :
    ...
    elif (final_flag == -1) and (Neg_score >= 4) and (Pos_score < 6) and (past >= 1) :
        trade_flag = trade_flag - 1
        if trade_flag < 0 :
            trade_flag = 0
        if (trade_flag < 4) :
            max_price = 0
            if n == -1 :
                past, cash2 = sell(past, cash, cash2, present_close, code, segment_percent, 1)
            else :
                past = 0
        elif n == -1 :
            post_message("upper전략에서는 trade_flag를 감소시켰지만 금일 매수,매도가 이뤄지지 않았습니다")

    elif n == -1 :
        post_message("upper전략에서는 금일 매수,매도가 이뤄지지 않았습니다")

    if n == -1 :
        return past, cash2, trade_flag, max_price, final_flag
    else :
        return past, trade_flag, max_price, final_flag
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.lower = QLabel("lower전략(X)", self)
        self.lower.move(20,nH)
        self.lower = QLineEdit("", self)
        self.lower.move(20 + 4 * nm, nH)

        self.lower_diff = QLineEdit("0", self)
        self.lower_diff.move(20 + 7 * nm, nH)
        ...
        self.mix = QLabel("mix전략(X)", self)
        self.mix.move(20,nH)
        self.mix = QLineEdit("", self)
        self.mix.move(20 + 4 * nm, nH)

        self.mix_diff = QLineEdit("0", self)
        self.mix_diff.move(20 + 7 * nm, nH)
        ...
        self.neutral = QLabel("neutral전략(X)", self)
        self.neutral.move(20,nH)
        self.neutral = QLineEdit("", self)
        self.neutral.move(20 + 4 * nm, nH)

        self.neutral_diff = QLineEdit("0", self)
        self.neutral_diff.move(20 + 7 * nm, nH)

        btnExit8 = QPushButton('미체결잔량취소', self)
        btnExit8.move(20 + 10 * nm, nH)
        btnExit8.clicked.connect(self.delete)
        ...
        self.upper.move(20 + 4 * nm, nH)

        self.upper_diff = QLineEdit("0", self)
        self.upper_diff.move(20 + 7 * nm, nH)


        nH += 50

        self.whole = QLabel("whole전략(X)", self)
        self.whole.move(20,nH)
        self.whole = QLineEdit("", self)
        self.whole.move(20 + 4 * nm, nH)

        self.whole_diff = QLineEdit("0", self)
        self.whole_diff.move(20 + 7 * nm, nH)
    ...
    def activate(self) :
        ...
        max_price = int(lines[7])
        final_flag = int(lines[8])
        f1.close()
        ...
        if (lower_diff > 0) & (lower_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("lower전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("lower전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        if (mix_diff > 0)  & (mix_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("mix전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("mix전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        if (neutral_diff > 0) & (neutral_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("neutral전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("neutral전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        if (upper_diff > 0) & (upper_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("upper전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("upper전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        if (whole_diff > 0) & (whole_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("whole전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("whole전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        if (buy_diff > 0) & (buy_past != 0):
            ...
            if trade_vol > 0 :
                ...
                post_message("buy전략 비중조정이 기존보유물량 부족으로 실행되지 않았습니다.")
            ...
            post_message("buy전략 비중조정이 보유물량 부족으로 실행되지 않았습니다.")
        ...
        if upper_percent != 0 :
            upper_past, cash2, trade_flag, max_price, final_flag = upper(upper_past, cash, cash2, present_close, code, percent, upper_percent, trade_flag, max_price, final_flag)
        if whole_percent != 0 :
        ...
        f2.write(str(int(lower_past)) + "," +str(int(mix_past)) + "," +str(int(neutral_past)) + "," +str(int(upper_past)) + "," +str(int(whole_past)) + "," +str(int(buy_past)) + "," +str(int(trade_flag)) + "," +str(int(max_price)) + "," +str(int(final_flag)) + "," + str(nowDate))
        f2.close()
    ...
    def messageBox(self):
        ...
        if result == QMessageBox.RestoreDefaults :
            ...
            for i in reversed(range(2, 32)) :
                past, trade_flag, max_price, final_flag = upper(past, 0, 0, 0, code, 0, 0, trade_flag, max_price, final_flag, -i)
            f3 = open(str(code) + ".txt", "w")
            f3.write("0,0,0,0,0,0," + str(trade_flag) + "," + str(max_price) + "," + str(final_flag))
            f3.close()
    ...
    def BlockRequestCancel(self, ordernum, code, amount):
        ...
        objCancelOrder.SetInputValue(2, cpTradeUtil.AccountNumber[0])  # 상품구분 - 주식 상품 중 첫번째
        objCancelOrder.SetInputValue(3, cpTradeUtil.GoodsList(cpTradeUtil.AccountNumber[0], 1)[0])  # 상품구분 - 주식 상품 중 첫번째
    ...
    def delete(self) :
        ...
        msgBox = QMessageBox()
        msgBox.setIcon(QMessageBox.NoIcon)
        msgBox.setWindowTitle("미체결잔량취소")

        msgBox.setText("< 귀하의 미체결잔량을 알려드립니다. >      ")

        if cnt1 == 0 :

            msgBox.setInformativeText("취소할 미체결잔량은 없습니다.")


        else :  

            for i in range(cnt1):
                order2 = CpTd5339.GetDataValue(1, i)  # 원주문번호
                code2 = CpTd5339.GetDataValue(3, i)  # 종목코드
                amount = CpTd5339.GetDataValue(6, i)  # 주문수량
                if i == 0 :
                    item2=[{'order' : order2, 'code': code2, 'amount': amount}]
                    item2 = pd.DataFrame(item2, columns=['order','code','amount'])
                else :
                    item2 = item2.append({'order' : order2, 'code': code2, 'amount': amount},ignore_index=True)
 
            for i in range(cnt1):

                self.BlockRequestCancel(item2.order[i], item2.code[i], item2.amount[i])
            
            msgBox.setInformativeText("미체결잔량을 전부 취소했습니다.")
