...
def get_signal(data):
    ...
    for i in range(len(data['close'])):
        ...
        if df.psychological_line.iloc[i] >= 0.7 :
        ...
        if (df.kdj_d.iloc[i] > 85) and (df.kdj_d.iloc[i] < df.kdj_j.iloc[i]):
            Neg_score = Neg_score + 2
        ...
        if (Pos_score >= 5)  and (asset > data['close'][i]): 
        ...
        elif (Neg_score >= 4) and (trade_flag >= 1) :
